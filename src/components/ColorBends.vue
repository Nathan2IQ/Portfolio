<template>
  <div ref="containerRef" :class="'color-bends-container ' + className"></div>
</template>
...
const handleResizeRef = ref(null)
...
onMounted(() => {
  ...
  handleResizeRef.value = handleResize
...
onUnmounted(() => {
  ...
  } else if (handleResizeRef.value) {
    window.removeEventListener('resize', handleResizeRef.value)
  }
...
.color-bends-container :deep(canvas) {
  display: block;
  width: 100%;
  height: 100%;
}
